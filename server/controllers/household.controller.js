...
const editHousehold = async (req, res) => {
  try {
    const { id, headId, relationship } = req.body;
    if (!id || !headId) {
      return res.status(400).json({ message: 'Missing id or headId' });
    }
    ...
    // Chỉ thay đổi nếu headId khác chủ hộ hiện tại
    if (householdFound.head?.toString() !== headId.toString()) {
      const oldHeadId = householdFound.head;

      // Xóa người mới khỏi danh sách thành viên (chuẩn bị lên chủ hộ)
      householdFound.members = householdFound.members.filter(
        m => m.member_id.toString() !== headId.toString()
      );

      // Chuyển chủ hộ cũ thành thành viên với quan hệ đã chọn
      if (oldHeadId && relationship) {
        householdFound.members.push({ member_id: oldHeadId, relation_to_head: relationship });
      }
