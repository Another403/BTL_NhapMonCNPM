...
const editProfile = async (req, res) => {
  try {
    // Loại bỏ các trường nhạy cảm — không cho phép thay đổi qua endpoint này
    const { password, _id, username, ...safeProfile } = req.body;
    const userId = req.cookies.token;
    ...
    Object.keys(safeProfile).forEach(key => {
      userFound[key] = safeProfile[key];
    });
    const { password: _pw, _id: _uid, ...json } = userFound._doc;
    await userFound.save();
